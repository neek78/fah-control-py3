...
class FAHControl(SingleAppServer):
    ...
    def __init__(self, glade = 'FAHControl.glade'):
        ...
        # URI hook
        #FIXME: re-instate
        #Gtk.LinkButton.set_uri_hook(self.on_uri_hook, None)
    ...
    def run(self):
        ...
        if sys.platform == 'darwin':
            ...
            try:
                ...
                ag.connect_group(key, mod, Gtk.AccelFlags.VISIBLE,
    ...
    def store_dimensions(self, widget, event, name):
        ...
        if 0 <= alloc.width and 0 <= alloc.height:
            self.db.set(name + '_width', alloc.width, queue = True);
            self.db.set(name + '_height', alloc.height, queue = True);
